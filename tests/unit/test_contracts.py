...
def evaluation(item: str = "item1", attempt: str = "attempt1") -> EvaluationResult:
    return EvaluationResult(
        ...
        feedback="Explain the index tradeoff",
        uncertain=True,
        rubric_version="r1",
        agent_version="a1",
        prompt_version="p1",
...
class ContractTests(unittest.TestCase):
    def test_rejects_extra_provider_fields_and_coerced_state(self):
        for payload in (
            {"student_id": "a", "state_version": "0"},
            {"student_id": "a", "state_version": 0, "admin": True},
        ):
    ...
    def test_batch_matches_exact_frozen_attempts(self):
        batch = InterviewBatchResult(
            session_id="s1", input_hash="a" * 64, evaluations=(evaluation(),), report="Feedback"
        )
    ...
    def test_rubric_and_answer_evidence_checked(self):
        kwargs = dict(
            expected_attempt="attempt1",
            expected_item="item1",
            expected_dimensions={"accuracy"},
            expected_rubric_version="r1",
            expected_agent_version="a1",
            expected_prompt_version="p1",
            answer="Use an index",
        )
    ...
    def test_success_requires_typed_action_output(self):
        ...
        with self.assertRaises(ValidationError):
            AgentTaskResult(
                action=Action.INTERVIEW_REPORT,
                request_key="r1",
                status=RunStatus.SUCCEEDED,
                provider_id="provider1",
                output=evaluation(),
    ...
    def test_plan_checks_capacity_and_state(self):
        ...
        plan = PlanDraft(
            ...
            items=(PlanItem(activity_id="a1", scheduled_date=date(2026, 9, 21), duration_minutes=30),),
        )
